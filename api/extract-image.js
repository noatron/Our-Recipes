...
function send(res, statusCode, data) {
    res.status(statusCode).json(typeof data === 'object' ? data : { error: data });
}
...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        if (!images.length) {
            return send(res, 400, { error: 'לא התקבלו תמונות' });
        }
...
החזר רק JSON תקין, בלי הסברים ובלי markdown.`;
        ...
        if (!jsonMatch) {
            return send(res, 200, { error: 'לא הצלחתי לחלץ מתכון מהתמונות' });
        }
        ...
        if (recipe.ingredients.length === 0 && recipe.instructions.length === 0) {
            return send(res, 200, { error: 'לא זוהו מרכיבים או הוראות בתמונה. נסי תמונה ברורה יותר (או צילום מקרוב של הטקסט) או הזיני את המתכון ידנית.' });
        }
        return send(res, 200, recipe);
    } catch (err) {
        console.error('extract-image error:', err);
        return send(res, 500, { error: 'שגיאה בחילוץ המתכון. נסי שוב.' });
    }
}
